...
Notion のコードブロックに楽天アフィリエイトのスクリプトを貼り付けることで、ブログ上で自動的にバナーとして表示されます。

*   **対応サービス**: 楽天アフィリエイト
*   **使用方法**:
    1.  Notion で「コード」ブロックを作成します。
    2.  楽天アフィリエイトから取得したスクリプトタグを貼り付けます。
    3.  ブログ上では自動的に iframe 内に展開され、バナーが表示されます。
*   **同期ブロック**: 同期ブロック内に配置した場合も正常に動作します。(2025/12/07 更新)
